...
function toTagList(v){
  // Accept: array of strings, comma/pipe separated string, null
  if(!v) return [];
  if(Array.isArray(v)) return v.map(x=> (x||'').toString().trim()).filter(Boolean);
  return (v||'').toString().split(/[,|]/).map(x=>x.trim()).filter(Boolean);
}
function hasTag(item, selected){
  if(!selected) return true;
  const sel = normalize(selected);
  const tags = toTagList(item.tags).map(normalize);
  return tags.includes(sel);
}
function collectAllTags(items){
  const set = new Map(); // normalized -> original (first seen)
  for(const it of items){
    for(const t of toTagList(it.tags)){
      const key = normalize(t);
      if(key && !set.has(key)) set.set(key, t);
    }
  }
  return Array.from(set.values()).sort((a,b)=>a.localeCompare(b,'ko'));
}
function populateTagSelect(selEl, tags){
  if(!selEl) return;
  const keep = selEl.value || '';
  selEl.innerHTML = '<option value="">증상(태그) 전체</option>' + tags.map(t=>`<option value="${t.replace(/"/g,'&quot;')}">${t}</option>`).join('');
  selEl.value = keep;
}
...
(async function main(){
  ...
  // Build tag list from items.json and fill the dropdown
  populateTagSelect(symptom, collectAllTags(all));
  ...
  function apply(){
    ...
    let view = all.filter(it => {
      ...
      const selectedTag = symptom ? symptom.value : '';
      if(selectedTag && !hasTag(it, selectedTag)) return false;
      return true;
    });
  ...
  if(symptom) symptom.addEventListener('change', apply);
